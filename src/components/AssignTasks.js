...
export const AssignTasks = ({handleBtnClick, employees, jobs, AssignTasks}) => {
    const [ current_job_id, set_Current_job_id ] = React.useState(null)
    const [ current_task_id, set_Current_task_id ] = React.useState(null)
    const [ current_employee_id, set_Current_employee_id ] = React.useState(null)
    ...
    return(
        <View>
            <Text style={styles.assign_main_title}>Assign Job</Text>
                    ...
                    <View>
                        {
                            cur_tasks && cur_tasks.tasks.map((item, index) => (
                                <TouchableOpacity key={index} style={styles.task_btn} onPress={() => { shiftView("employee_listing"); set_Current_task_id(item.id) }}>
                    ...
                    <View>
                        {
                            unassigned_employees && unassigned_employees.map((item, index) => (
                                <TouchableOpacity key={index} style={styles.employee_btn} onPress={() => { 
                                                    shiftView("job_listing"); 
                                                    set_Current_employee_id(item.id);  
                                                    AssignTasks(current_job_id, current_task_id, item.id);    
                                                    // AssignTasks();    
                                                }}>
                                    <Text>{item.name}</Text>
                                </TouchableOpacity>
